...
@description('Allowed CORS origins for the Wolfman client (local dev and deployed PWA).')
param corsAllowedOrigins array = []

@secure()
@description('Azure Maps subscription key for places search. Empty disables that feature.')
...
@description('Azure Communication Services SMS from-number, e.g. +18005551234.')
param acsSmsFromNumber string = ''

@secure()
@description('Shared key required in the x-wolfman-key header to send SMS. Empty disables that check (not recommended).')
param wolfmanApiKey string = ''
...
output API_BASE_URL string = 'https://${api.outputs.SERVICE_API_NAME}.azurewebsites.net'
