...
function AssistantView() {
  ...
  return <div className="view assistant-view"><header className="assistant-heading"><span className={`wolfman-orb ${voice.listening ? 'listening' : ''}`}><Bot size={22} /></span><div><h1>Wolfman</h1><p><span className="online-dot" /> {voice.enabled ? voice.status : busy ? 'Reading requested sources' : 'Ready'}</p></div><button className={`voice-button ${voice.enabled ? 'enabled' : ''}`} onClick={voice.toggle} disabled={!voice.supported} aria-pressed={voice.enabled} title={voice.supported ? 'Toggle Wolfman voice activation' : 'Voice recognition is unavailable'}>{voice.enabled ? <Mic size={18} /> : <MicOff size={18} />}<span>{voice.enabled ? 'Voice on' : 'Voice off'}</span></button></header><div className="chat-thread">{messages.map((message) => <div key={message.id} className={`message ${message.role}`}><ReactMarkdown>{message.content}</ReactMarkdown></div>)}</div><form className="composer" onSubmit={(event) => { event.preventDefault(); void send(input) }}><input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Type a request or include a public URL..." aria-label="Message Wolfman" /><button className="send-button" type="submit" title="Send" disabled={busy}><Send size={18} /></button></form><p className="disclaimer">Wolfman reads only the information requested in each message.</p></div>
}
